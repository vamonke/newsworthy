...
import { LIMITS, emptyState, sweep, reserve, attach, register, release, releaseIp, nextWake } from './gate-core.js';
...
export class Gate extends DurableObject {
  constructor(ctx, env) {
    ...
    ctx.blockConcurrencyWhile(async () => { this.state = (await ctx.storage.get('state')) || emptyState(); });
  }

  async save() {
    await this.ctx.storage.put('state', this.state);
    const wake = nextWake(this.state);
  ...
  async open(ip) {
    ...
    const held = reserve(this.state, ip, Date.now(), this.limits);
    await this.save();
